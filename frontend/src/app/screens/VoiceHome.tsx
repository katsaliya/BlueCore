...
const shiftReplies = [
  "Good to know. I'll keep checking the acoustic readings quietly. Just say 'SEREN' any time you need me.",
...
function WireframeOrb({
  ...
  useEffect(() => {
    ...
    // Smooth noise via layered sines — gives organic blob deformation
    ...
    const draw = () => {
      ...
      const lineColor = (z: number, baseAlpha: number) => {
        const depth = (z + BASE_R * 1.8) / (BASE_R * 3.6); // 0..1
        const a = baseAlpha * (0.25 + depth * 0.75);
        // Parse orbColor to extract RGB values, or use a simple alpha overlay
      ...
      for (let i = 0; i <= ROWS; i++) {
        ...
        // avg z for this ring
        const avgZ = pts[idx(i, 0)][2];
        ctx.strokeStyle = lineColor(avgZ, 0.55);
        ctx.lineWidth = 0.45;
        ctx.stroke();
      }
      ...
      for (let j = 0; j <= COLS; j++) {
        ...
        const avgZ = pts[idx(ROWS / 2, j)][2];
        ctx.strokeStyle = lineColor(avgZ, 0.45);
        ctx.lineWidth = 0.4;
        ctx.stroke();
      }

      // Draw triangulation diagonals (every other cell for density)
      for (let i = 0; i < ROWS; i++) {
        for (let j = 0; j < COLS; j += 1) {
          ...
          ctx.strokeStyle = lineColor(avgZ, 0.22);
          ctx.lineWidth = 0.35;
          ctx.stroke();
        }
      }
      ...
      for (let i = 0; i < ROWS; i++) {
        for (let j = 0; j < COLS; j += 2) {
          ...
          ctx.moveTo(CX + x1, CY + y1);
          ctx.lineTo(CX + x2, CY + y2);
          ctx.strokeStyle = lineColor(avgZ, 0.15);
          ctx.lineWidth = 0.3;
          ctx.stroke();
        }
      }
...
export function VoiceHome() {
  ...
  const accentHex = useCssVar(
    "--app-accent",
    theme === "light" ? "#25467f" : "#4fc3f7",
    [theme],
  );
  ...
  // Amplitude MotionValue → ref for canvas
  ...
  const getReplies = useCallback(() => {
    return ctx.mode === "break" ? breakReplies : ctx.mode === "on-shift" ? shiftReplies : offShiftReplies;
  }, [ctx.mode]);
  ...
  const modeLabel = ctx.mode === "break" ? "Break Check-in" : ctx.mode === "on-shift" ? "On Watch" : "Off Watch";
  const modeDotColor =
    ctx.mode === "break"
  ...
  return (
    <div className="flex flex-col h-screen relative overflow-hidden bg-app-canvas">
      ...
      {/* ── Chat messages (full height, behind orb) */}
      <div
        className="absolute inset-0 overflow-y-auto flex flex-col justify-end px-5 pb-40 pt-32"
        ref={scrollRef}
                ...
                style={
                  msg.role === "user"
                    ...
                    : {
                        ...
                        border: "1px solid var(--app-accent-border-20)",
                        color: "var(--app-fg)",
      ...
      {/* ── Top bar with frosted overlay */}
      <div
        className="flex-shrink-0 flex items-center justify-between px-5 pt-12 pb-4 relative z-20"
        style={{
          background: "var(--app-home-header-scrim)",
      ...
      >
        <div>
          ...
          <div className="flex items-center gap-2 mt-1">
            ...
            <span className="text-[11px] tracking-widest uppercase" style={{ color: modeLabelColor, opacity: 0.75 }}>
              {modeLabel}
            </span>
          </div>
        </div>
      ...
      <AnimatePresence>
        {showNav && (
          <motion.div
            ...
            style={{
              background: "var(--app-home-panel-bg)",
              backdropFilter: "blur(24px)",
              border: "1px solid var(--app-accent-border-20)",
              boxShadow: "var(--app-home-panel-shadow)",
            }}
          >
            {/* Section label */}
            <div className="px-5 pt-4 pb-2">
              <span className="text-[10px] tracking-widest uppercase" style={{ color: "var(--app-fg-muted)" }}>
                START A DOCUMENT
              </span>
            </div>
            ...
            <button
              ...
              style={{ borderBottom: "1px solid var(--app-accent-border-10)" }}
            >
              <div
              ...
              <div className="flex-1 text-left">
                <div className="text-sm" style={{ color: "var(--app-fg)" }}>Voyage Log</div>
                <div
                  className="inline-block mt-0.5 px-2 py-0.5 rounded-full text-[10px]"
                  style={{ background: "var(--app-warning-bg)", color: "var(--app-warning-fg)" }}
                >
                  Due today
                </div>
              </div>
              <ChevronRight size={13} style={{ color: "var(--app-fg-muted)" }} />
            </button>

            {/* Overdue document row */}
            <button
              onClick={() => setShowNav(false)}
              className="w-full flex items-center gap-3 px-5 py-3.5"
              style={{ borderBottom: "1px solid var(--app-accent-border-10)" }}
            >
              ...
              <div className="flex-1 text-left">
                <div className="text-sm" style={{ color: "var(--app-fg)" }}>Engine Room Log</div>
                <div
                  className="inline-block mt-0.5 px-2 py-0.5 rounded-full text-[10px]"
            ...
            {/* Resume session row (conditional) */}
            <button
              onClick={() => setShowNav(false)}
              className="w-full flex items-center gap-3 px-5 py-3.5"
              style={{
                borderBottom: "1px solid var(--app-accent-border-10)",
                background: "var(--app-row-highlight-bg)",
              }}
            >
              <div
              ...
              >
                <Play size={14} style={{ color: "var(--app-fg-muted)" }} />
                <motion.div
              ...
              <div className="flex-1 text-left">
                <div className="text-sm" style={{ color: "var(--app-fg)" }}>Resume: Port Arrival Report</div>
                <div className="text-xs mt-0.5" style={{ color: "var(--app-fg-muted)" }}>
                  8 of 14 fields complete
                </div>
              </div>
              <ChevronRight size={13} style={{ color: "var(--app-fg-muted)" }} />
            </button>
            ...
            >
              ...
              >
                <Grid size={14} style={{ color: "var(--app-fg-muted)" }} />
              </div>
              <div className="text-sm" style={{ color: "var(--app-fg-muted)" }}>Browse all documents</div>
              <ChevronRight size={13} className="ml-auto" style={{ color: "var(--app-fg-muted)" }} />
            </button>
          </motion.div>
        )}
      </AnimatePresence>

      {/* ── Orb with frosted background */}
      <div
        className="absolute inset-0 flex items-center justify-center z-10 pointer-events-none"
        style={{ background: "var(--app-home-orb-vignette)" }}
      >
        <motion.div
        ...
        >
          <button onClick={handleMicToggle} className="focus:outline-none relative">
            ...
            <div
              ...
              style={{
                background: "var(--app-orb-glow)",
                filter: "blur(20px)",
                transform: "scale(1.2)",
              }}
            />
            <WireframeOrb state={orbState} ampRef={ampRef} orbColor={accentHex} />
                ...
                <>
                  {[1, 2].map((r) => (
                    <motion.div
                      ...
                      animate={{ scale: 1 + r * 0.22, opacity: 0 }}
      ...
      {/* ── Bottom controls */}
      <div className="flex-shrink-0 relative z-20 pb-2 pt-1 px-5">
        <div className="flex items-center gap-3">
          <AnimatePresence>
            {!showInput && (
              <motion.button
              ...
              >
                <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                  <rect x="1" y="3" width="14" height="2" rx="1" fill="var(--app-fg-muted)" />
                  <rect x="1" y="7" width="10" height="2" rx="1" fill="var(--app-fg-muted)" opacity="0.6" />
                  <rect x="1" y="11" width="12" height="2" rx="1" fill="var(--app-fg-muted)" opacity="0.6" />
                </svg>
              </motion.button>
            )}
          </AnimatePresence>
          ...
          <motion.button
            ...
            style={{
              width: showInput ? 46 : 60,
              height: showInput ? 46 : 60,
          ...
          >
            <Mic
              size={showInput ? 18 : 22}
              style={{ color: isListening ? "var(--app-accent)" : "var(--app-fg-muted)" }}
              fill={isListening ? accentHex : "none"}
            />
            {isListening && (
              <motion.div
      ...
      {/* ── Bottom navigation */}
      <nav className="flex-shrink-0 relative z-20 px-4 pb-6 pt-3 bg-app-nav border-t border-app-nav-border">
